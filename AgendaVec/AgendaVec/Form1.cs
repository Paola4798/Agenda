...
namespace AgendaVec
{
    public partial class Form1 : Form
    {
        ...
        {
            if (txtNombre.Text == "" || txtTelefono.Text == "" || txtCorreo.Text == "" || txtFecha.Text == "")
            ...
            else
            {
                if (contactos._posicionActual < contactos.vec.Length)
                {
                   ...
                   string vecCorreo =Convert.ToString( txtCorreo.Text);
                    ...
                    contactos.agregar(new Persona(vecNombre, vecTelefono, vecCorreo, vecFecha));
                }
                else
                {
        ...
        private void btnInsertar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (contactos._posicionActual < contactos.vec.Length)
                {
                    ...
                    contactos.insertar(new Persona(vecNombre, vecTelefono, vecCorreo, vecFecha), Convert.ToInt16(txtPosicion.Text));
                }
                else
                {
                    MessageBox.Show("No hay mas espacio en la agenda!");
